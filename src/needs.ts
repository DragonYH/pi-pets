 ...
 * When working: extreme lows (<20) override working, otherwise show working.
 * When not working: falls through to original emotionFromNeeds logic.
 */
export function resolveEmotion(needs: Needs, isRecentlyWorking: boolean): EmotionState {
  ...
  if (isRecentlyWorking) {
    // Extreme lows still show needs during work
    if (energy < 20) {
      return 'tired';
    }
    if (hunger < 20) {
